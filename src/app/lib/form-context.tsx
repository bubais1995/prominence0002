...
const createDefaultData = (): ApplicationData => ({
  type: "personal",
  accountTypeId: "",
...
export function FormProvider({ children }: { children: ReactNode }) {
  ...
  const [data, setData] = useState<ApplicationData>(() => createDefaultData());
  ...
  const setType = (type: ApplicationType) => {
    setData({ ...createDefaultData(), type, applicationId: crypto.randomUUID().substring(0, 8) });
    setCurrentStep(1);
  };
